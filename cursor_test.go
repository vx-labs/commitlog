...
func TestCursor(t *testing.T) {
	...
	t.Run("should allow seeking position in cursor", func(t *testing.T) {
		...
		t.Run("start", func(t *testing.T) {
			...
			require.Equal(t, int64(64), r.pos)
			offset, _ = r.Seek(31, io.SeekStart)
			require.Equal(t, int64(32), r.pos)
			require.Equal(t, int64(31), offset)
			offset, _ = r.Seek(0, io.SeekStart)
			require.Equal(t, int64(0), r.pos)
			require.Equal(t, int64(0), offset)
			offset, _ = r.Seek(200, io.SeekStart)
			require.Equal(t, int64(320), r.pos)
			require.Equal(t, int64(50), offset)
			require.Equal(t, uint64(40), r.currentSegment.BaseOffset())
		})
	})
